...
class Dispatch:
    ...
    @staticmethod
    def add_command(bot, source, message, whisper, args):
        ...
        if len(message_parts) < 2:
            bot.private_message(source, "Usage: !add command ALIAS [options] RESPONSE")
            return False

        options, response = bot.commands.parse_command_arguments(message_parts[1:])

        if options is False:
            bot.private_message(source, "Invalid command")
            return False
        ...
        if new_command is True:
            bot.private_message(source, f"Added your command (ID: {command.id})")
    ...
    def edit_command(bot, source, message, event, args):
        ...
        if message:
            ...
            bot.private_message(source, f"Updated the command (ID: {command.id})")
    ...
    def add_alias(bot, source, message, event, args):
        ...
        if message:
            ...
            message_parts = message.split()
            if len(message_parts) < 2:
                bot.private_message(source, "Usage: !add alias existingalias newalias")
                return False
            ...
            if existing_alias not in bot.commands:
                bot.private_message(source, f'No command called "{existing_alias}" found')
                return False
            ...
            if command.data is None or command.command is None:
                bot.private_message(source, "That command cannot have aliases added to.")
                return False
            ...
            if len(added_aliases) > 0:
                ...
                bot.private_message(source, f"Successfully added the aliases {', '.join(added_aliases)} to {existing_alias}")
            ...
            if len(already_used_aliases) > 0:
                bot.private_message(source, f"The following aliases were already in use: {', '.join(already_used_aliases)}")
        else:
            bot.private_message(source, "Usage: !add alias existingalias newalias")
    ...
    def remove_alias(bot, source, message, event, args):
        ...
        if message:
            ...
            log.info(aliases)
            if len(aliases) < 1:
                bot.private_message(source, "Usage: !remove alias EXISTINGALIAS")
                return False
            ...
            for alias in aliases:
                ...
                # aliases registered. (command.command and command.data are None on those commands)
                if command.data is None or command.command is None:
                    bot.private_message(source, "That command cannot have aliases removed from.")
                    return False
                ...
                if len(current_aliases) == 0:
                    bot.private_message(source, f"{alias} is the only remaining alias for this command and can't be removed.")
                    continue
    ...
    def remove_command(bot, source, message, event, args):
        if message:
            ...
            if command is None:
                bot.private_message(source, "No command with the given parameters found")
                return False

            if command.id == -1:
                bot.private_message(source, "That command is an internal command, it cannot be removed.")
                return False

            if source.level < 2000:
                if command.action is not None and not command.action.type == "message":
                    bot.private_message(source, "That command is not a normal command, it cannot be removed by you.")
                    return False

            bot.private_message(source, f"Successfully removed command with id {command.id}")
            ...
            bot.private_message(source, "Usage: !remove command (COMMAND_ID|COMMAND_ALIAS)")
