...
class Dispatch:
    ...
    @staticmethod
    def add_command(bot, source_id, message, whisper, args):
        ...
        if len(message_parts) < 2:
            bot.private_message(source_id, "Usage: !add command ALIAS [options] RESPONSE")
            return False

        options, response = bot.commands.parse_command_arguments(message_parts[1:])

        if options is False:
            bot.private_message(source_id, "Invalid command")
            return False

        options["added_by"] = source_id
        ...
        if new_command is True:
            bot.private_message(source_id, f"Added your command (ID: {command.id})")

            log_msg = f"The !{command.command.split('|')[0]} command has been created"
            # AdminLogManager.add_entry("Command created", source_id, log_msg)
            return True
    ...
    @staticmethod
    def edit_command(bot, source_id, message, event, args):
        ...
        if message:
            ...
            options["edited_by"] = source_id

            if options is False:
                bot.whisper(source_id, "Invalid command")
                return False
            ...
            bot.private_message(source_id, f"Updated the command (ID: {command.id})")
            ...
            # AdminLogManager.add_entry(
                # "Command edited", source_id, log_msg, data={"old_message": old_message, "new_message": new_message}
            # )

    @staticmethod
    def add_funccommand(bot, source_id, message, event, args):
        ...
        if message:
            ...
            message_parts = message.split(" ")
            if len(message_parts) < 2:
                bot.whisper(source_id, "Usage: !add funccommand ALIAS [options] CALLBACK")
                return False
            ...
            command, new_command, alias_matched = bot.commands.create_command(alias_str, action=action, **options)
            if new_command is True:
                bot.whisper(source_id, f"Added your command (ID: {command.id})")
                return True
    ...
    @staticmethod
    def edit_funccommand(bot, source_id, message, event, args):
        ...
        if message:
            ...
            if len(action["cb"]) > 0:
                options["action"] = action
            bot.commands.edit_command(command, **options)
            bot.whisper(source_id, f"Updated the command (ID: {command.id})")

    @staticmethod
    def add_alias(bot, source_id, message, event, args):
        ...
        if message:
            ...
            message_parts = message.split()
            if len(message_parts) < 2:
                bot.private_message(source_id, "Usage: !add alias existingalias newalias")
                return False
            ...
            if existing_alias not in bot.commands:
                bot.private_message(source_id, f'No command called "{existing_alias}" found')
                return False
            ...
            if command.data is None or command.command is None:
                bot.private_message(source_id, "That command cannot have aliases added to.")
                return False
            ...
            if len(added_aliases) > 0:
                ...
                bot.private_message(source_id, f"Successfully added the aliases {', '.join(added_aliases)} to {existing_alias}")
                log_msg = f"The aliases {', '.join(added_aliases)} has been added to {existing_alias}"
                # AdminLogManager.add_entry("Alias added", source_id, log_msg)
            if len(already_used_aliases) > 0:
                bot.private_message(source_id, f"The following aliases were already in use: {', '.join(already_used_aliases)}")
        else:
            bot.private_message(source_id, "Usage: !add alias existingalias newalias")

    @staticmethod
    def remove_alias(bot, source_id, message, event, args):
        ...
        if message:
            ...
            log.info(aliases)
            if len(aliases) < 1:
                bot.private_message(source_id, "Usage: !remove alias EXISTINGALIAS")
                return False
            ...
            for alias in aliases:
                ...
                # aliases registered. (command.command and command.data are None on those commands)
                if command.data is None or command.command is None:
                    bot.private_message(source_id, "That command cannot have aliases removed from.")
                    return False
                ...
                if len(current_aliases) == 0:
                    bot.private_message(source_id, f"{alias} is the only remaining alias for this command and can't be removed.")
                    continue
                ...
                # AdminLogManager.add_entry("Alias removed", source_id, log_msg)
            ...
            if len(whisper_str) > 0:
                bot.whisper(source_id, whisper_str)
        else:
            bot.private_message(source_id, "Usage: !remove alias EXISTINGALIAS")

    @staticmethod
    def remove_command(bot, source_id, message, event, args):
        if message:
            id = None
            command = None
            try:
            ...
            if command is None:
                bot.private_message(source_id, "No command with the given parameters found")
                return False

            if command.id == -1:
                bot.private_message(source_id, "That command is an internal command, it cannot be removed.")
                return False

            if source_id.level < 2000:
                if command.action is not None and not command.action.type == "message":
                    bot.private_message(source_id, "That command is not a normal command, it cannot be removed by you.")
                    return False

            bot.private_message(source_id, f"Successfully removed command with id {command.id}")
            log_msg = f"The !{command.command.split('|')[0]} command has been removed"
            # AdminLogManager.add_entry("Command removed", source_id, log_msg)
            bot.commands.remove_command(command)
        else:
            bot.private_message(source_id, "Usage: !remove command (COMMAND_ID|COMMAND_ALIAS)")
