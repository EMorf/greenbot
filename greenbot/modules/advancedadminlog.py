...
class AdvancedAdminLog(BaseModule):
    ...
    async def message_delete(self, payload):
        if not self.settings["log_delete_message"]:
            return
        out_channel = list(self.bot.filters.get_channel(
        ...
        sent_in_channel = list(self.bot.filters.get_channel([int(payload.channel_id)], None))[0]
        channels = (
    ...
    async def message_edit(self, payload):
        ...
        sent_in_channel = list(self.bot.filters.get_channel(
            [int(payload.data["channel_id"])], None
        ))[0]
        channels = (
            self.settings["ingore_channels"].split(" ")
            if self.settings["ingore_channels"] != ""
